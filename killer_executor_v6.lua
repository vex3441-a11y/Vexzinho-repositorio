-- KILLER EXECUTOR V6.0
-- Executor com Backdoor Funcional e Execução Múltipla
...
-- Ícone de caveira e som (CORRIGIDO)
local skullIcon = "rbxassetid://73913603124530"
...
-- Função para executar código diretamente
...
-- Função para tentar executar em TODOS os backdoors
local function executeOnAllBackdoors(script)
    local successCount = 0
    local totalAttempts = 0
    
    -- Tenta no backdoor criado primeiro
    ...
    -- Tenta em TODOS os backdoors encontrados
    for _, bd in ipairs(foundBackdoors) do
        totalAttempts = totalAttempts + 1
        
        if bd.type == "bindable" then
            local success = pcall(function()
                bd.remote:Fire(script)
            end)
            if success then
                successCount = successCount + 1
            end
        else
            -- Tenta múltiplos métodos para RemoteEvents
            local methods = {
                function() bd.remote:FireServer(script) end,
                function() bd.remote:FireServer(loadstring(script)) end,
                function() bd.remote:FireServer("Execute", script) end,
                function() bd.remote:FireServer(LocalPlayer, script) end,
                function() bd.remote:FireServer({code = script}) end,
            }
...
-- Função para criar backdoor funcional
local function createCustomBackdoor()
    ...
    local success = pcall(function()
        ...
        local bindableEvent = ReplicatedStorage:FindFirstChild(backdoorName .. "_Trigger")
        
        if not bindableEvent then
            wait(1)
            bindableEvent = ReplicatedStorage:FindFirstChild(backdoorName .. "_Trigger")
        end
        ...
        if bindableEvent then
            ...
            updateStatus("⚠️ Backdoor criado, aguarde...", Color3.fromRGB(255, 150, 50))
            notify("⚠️ PROCESSO", "Backdoor em inicialização...", 3)
        end
    end)
...
function addBackdoorToList(name, remotePath, remoteType)
    ...
    UseButton.MouseButton1Click:Connect(function()
        ...
        if remoteType == "bindable" then
            ...
            local success = false
            local methods = {
                function() remotePath:FireServer(script) end,
                function() remotePath:FireServer(loadstring(script)) end,
                function() remotePath:FireServer("Execute", script) end,
                function() remotePath:FireServer(LocalPlayer, script) end,
            }
            ...
            if success then
                ...
                notify("✅ ENVIADO", "Script enviado!", 3)
            else
                updateStatus("⚠️ Tentativa concluída", Color3.fromRGB(255, 150, 50))
                notify("⚠️ PROCESSADO", "Tentativa realizada!", 3)
            end
        end
    end)
...
-- Função para escanear backdoors
local function scanForBackdoors()
                ...
                if obj:IsA("RemoteEvent") or obj:IsA("RemoteFunction") then
                    for _, name in ipairs(backdoorNames) do
                        if string.find(obj.Name:lower(), name:lower()) then
                            table.insert(foundBackdoors, {name = obj.Name, remote = obj, type = "remote"})
                            break
                        end
                    end
    ...
    searchInContainer(game.Workspace)
    pcall(function() searchInContainer(game.Lighting) end)
    pcall(function() searchInContainer(game.ServerStorage) end)
    
    wait(2)
    
    if #foundBackdoors > 0 then
        updateStatus("✅ " .. #foundBackdoors .. " backdoor(s) encontrado(s)!", Color3.fromRGB(50, 200, 50))
        notify("🔓 ENCONTRADOS", #foundBackdoors .. " backdoors detectados!", 4)
        ...
        notify("⚠️ NADA", "Nenhum backdoor no jogo!", 4)
    end
end
...
-- Botão Executar (USA TODOS OS BACKDOORS)
ExecuteBtn.MouseButton1Click:Connect(function()
    ...
    updateStatus("⚡ Executando em TODOS os backdoors...", Color3.fromRGB(255, 200, 50))
    
    local successCount, totalAttempts = executeOnAllBackdoors(script)
    
    wait(0.5)
    
    if successCount > 0 then
        updateStatus("✅ Executado em " .. successCount .. "/" .. totalAttempts .. " backdoors!", Color3.fromRGB(50, 200, 50))
        notify("✅ SUCESSO", "Executado em " .. successCount .. " backdoors!", 4)
    elseif totalAttempts > 0 then
        updateStatus("⚠️ Tentativas: " .. totalAttempts .. " backdoors", Color3.fromRGB(255, 150, 50))
        notify("⚠️ ENVIADO", "Script processado!", 3)
    else
        -- Se não tem backdoors, executa localmente
        ...
        if success then
            ...
            notify("✅ LOCAL", "Script executado!", 3)
        else
            updateStatus("❌ Erro: " .. tostring(err), Color3.fromRGB(200, 50, 50))
            notify("❌ ERRO", "Falha na execução!", 4)
        end
    end
end)
...
ClearBtn.MouseButton1Click:Connect(function()
    ...
    ScriptBox.Text = ""
    updateStatus("🗑️ Limpo!", Color3.fromRGB(150, 150, 150))
...
updateStatus("✅ Pronto para usar!", Color3.fromRGB(50, 200, 50))
