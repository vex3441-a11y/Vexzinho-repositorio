-- KILLER EXECUTOR V4.0
-- Executor com Sistema de Criação de Backdoor
...
-- Função para criar backdoor customizado
local function createCustomBackdoor()
    updateStatus("🔧 Criando backdoor...", Color3.fromRGB(255, 200, 50))
    
    pcall(function()
        -- Criar o RemoteEvent backdoor
        local backdoor = Instance.new("RemoteEvent")
        backdoor.Name = "KillerBackdoor_" .. math.random(1000, 9999)
        
        -- Tentar colocar em ReplicatedStorage
        local success = pcall(function()
            backdoor.Parent = ReplicatedStorage
        end)
        
        if not success then
            -- Se falhar, tenta no Workspace
            backdoor.Parent = game.Workspace
        end
        
        -- Criar o script de recepção
        ...
        table.insert(foundBackdoors, {
            name = backdoor.Name .. " [CRIADO]",
            path = backdoor:GetFullName(),
            remote = backdoor
        })
        
        -- Adicionar visualmente à lista
        addBackdoorToList(backdoor.Name .. " [CRIADO]", backdoor)
        
        wait(1)
        updateStatus("✅ Backdoor criado: " .. backdoor.Name, Color3.fromRGB(50, 200, 50))
        notify("✅ SUCESSO", "Backdoor criado com sucesso!", 4)
    end)
end

-- Função para adicionar backdoor à lista
function addBackdoorToList(name, remotePath)
    ...
    UseButton.MouseButton1Click:Connect(function()
        ...
        notify("✅ EXECUTADO", "Script enviado para o servidor!", 3)
    end)
...
local function scanForBackdoors()
                ...
                if obj:IsA("RemoteEvent") or obj:IsA("RemoteFunction") then
                    for _, name in ipairs(backdoorNames) do
                        if string.find(obj.Name:lower(), name:lower()) then
                            table.insert(foundBackdoors, {name = obj.Name, path = obj:GetFullName(), remote = obj})
                            break
                        end
                    end
    ...
    if #foundBackdoors > 0 then
        ...
        for _, bd in ipairs(foundBackdoors) do
            addBackdoorToList(bd.name, bd.remote)
        end
    else
...
ExecuteBtn.MouseButton1Click:Connect(function()
    ...
    if #foundBackdoors == 0 and not createdBackdoor then
        notify("⚠️ AVISO", "Crie ou escaneie backdoors primeiro!", 3)
        updateStatus("⚠️ Nenhum backdoor disponível!", Color3.fromRGB(200, 150, 50))
        return
    end
    ...
    -- Tenta usar o backdoor criado primeiro
    if createdBackdoor then
        pcall(function()
            createdBackdoor:FireServer(script)
            success = true
        end)
    end
    
    -- Se não deu certo, tenta os outros
    if not success then
        for _, bd in ipairs(foundBackdoors) do
            pcall(function()
    ...
    if success then
        ...
        notify("⚠️ PROCESSADO", "Script enviado!", 3)
    end
end)
